...
- **Two-stage AST modeling**: mixed-effects model + kernel-smoothed residuals across age, space, and time
- **Two backends**: frequentist (lme4, default) and Bayesian (cmdstanr, experimental)
...
- **Uncertainty quantification**: bootstrap (frequentist) or posterior credible intervals (Bayesian)
...
See the [full tutorial](https://choxos.github.io/dmft/articles/tutorial.html) for a complete walkthrough.
...
The package uses a two-stage AST (Age-Spatial-Temporal) approach following Shoaee et al. (2022, 2025):
...
The frequentist backend estimates parameters via REML (lme4). The Bayesian backend uses HMC/NUTS sampling (Stan) with weakly informative priors.
